...
library(devtools)
devtools::install_github("activityMonitoring/epicoda",  build_opts = c("--no-resave-data"), build_vignettes = TRUE, build_manual = TRUE) # TROUBLESHOOTING if this doesn't work, use build_vignettes = FALSE. You have all the info from the vignette here!
```
...
# A basic analysis
This tutorial illustrates some steps of an epidemiological analysis with a compositional exposure. It uses a package, `epicoda`, developed for this purpose. More information about any function we use can be found by running `?function_name` (as usual).
...
The default in the function, if neither `rounded_zeroes` or `det_limit` is specified, is to impute them with the detection limit (`det_limit`) set as the minimum non-zero value observed in the data.
