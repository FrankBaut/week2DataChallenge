...
The following code constructs a table to describe accelerometer measured attributes by conventional groupings (e.g. age, sex, self-rated health, season). It does analysis of variance, adjusted for the variables listed as confounder columns.
...
# specify analysis of variance confounder columns
...
html <- paste(html, 'Analysis of variance test used to compare metrics between groups')
...
hist(dAll$fu_time[(dAll$UkBiobankAssessCent %in% c("Cardiff", "Wrexham", "Swansea"))&(dAll$died == 1)])
```
